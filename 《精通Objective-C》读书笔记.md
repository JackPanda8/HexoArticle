...
1. 运行时系统的组成部分：编译器（当源代码使用了系统的动态特性，编译器会为之**生成**相应的调用运行时库系统的代码）和运行时系统库（实现OC的动态特性）。

2. **实例对象**的isa指向**类**（对象），**类**的isa指向**元类**（对象），**所有元类**（包括根元类本身）的isa指向**根元类**（即NSObject类的元类，NSObject的isa指向的类）。

3. dyld（the dynamic link editor ）动态链接器

   >iOS 中用到的所有系统 framework 都是动态链接的，类比成插头和插排，静态链接的代码在编译后的静态链接过程就将插头和插排一个个插好，运行时直接执行二进制文件；而动态链接需要在程序启动时去完成“插插销”的过程，所以在我们写的代码执行前，动态连接器需要完成准备工作。 ---引用自sunny博客

4. 与运行时系统进行交互的三个层级：

   通过Objective-C**源代码**：当编译包含OC类和方法的代码时，编译器创建实现了语言动态特性的数据结构和函数的调用。类、类别、协议，实例变量模板、消息传递等都会依赖运行时系统实现。

   通过**NSObjec**t方法：自省有关的方法。

   通过系统提供的**运行时系统API**：通过包含#import<objc/runtime.h>，运行时系统是一个动态共享库，包含公共接口组成的一组函数和数据结构，其头文件位于目录/usr/include/objc。 

---

### 第九章 专家级技巧：使用运行时系统API    

1.  当消息的接受者在访问过自身（以及父类）的方法缓存、方法列表后，仍旧无法响应消息时，会进入运行时的**动态方法决议**和**消息转发**流程，如下图：

   ![](https://raw.githubusercontent.com/JackPanda8/ImageHosting/master/1449456989807643.png)

---

### 第十章 Foundation框架中的常用类
